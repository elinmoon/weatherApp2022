$(document).ready(function(){
    //8c82bae1fd4c0405121ce6d18104f0f3

    //https://api.openweathermap.org/data/2.5/weather?q=seoul&appid=8c82bae1fd4c0405121ce6d18104f0f3


    var cityName = [];  //버튼 클릭시 또는 검색바에서 입력 후 엔터를 누를 시, 도시명 데이터를 수집한다.
    console.log(cityName); //[]

    var myAPI = `8c82bae1fd4c0405121ce6d18104f0f3`;
    ...
    function w_info(){
        ...
        //appen(), prepedn() 자식을 추가하는 메서드는 기존 자식을 남기는 상태에서 새로운 자식들을 추가 ==> 기존 데이터를 새롭게 반복하여 적용하고자 할 때는 반복문 이전에 초기화가 필요
        ...
        }); //$("#weather ul li").each(function() : 종료
    }


    $(document).on("click", ".cities button", function(){
        console.log(this); //<button>Paris</button>
        var city_txt = $(this).text();
        console.log(city_txt); //Paris

        cityName.unshift(city_txt); //cityName이라는 전역 변수에 배열 데이터의 첫번째 인덱스에 추출한 텍스트(도시명)를 넣어준다.
        console.log(cityName); //['Daegu', 'Incheon', 'Tokyo', 'Suwon-si'] -> 에러 나지만, click 할때 마다 배열 데이터에 쌓인다.
    ...
    navigator.geolocation.getCurrentPosition(function(position){
        ...
        $.ajax({
            ...
            success : function(data){
                ...
                cityName.unshift(city); //브라우저가 로딩하자마자 현재 나의 위치를 배열 데이터의 넣는다.
                w_info(); //함수문을 호출합니다 => 배열데이터의 개수에 따라 구조를 구정하고 그 데이터 내용을 넣겠다는 의미
            }
        });
